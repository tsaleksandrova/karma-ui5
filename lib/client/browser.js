...
(function(window) {
	...
	// TODO add event listener "on info" - get screenshot data and push it to customData

	karma.start = function() {
		// will contain misc data about the test (incl screenshot) that is collected and sent on completion
		const customData = [];
			...
			function runTestPage(i) {
				...
				windowUtil(qunitHtmlFile, function(testWindow) {
					...
					QUnit.testDone(function(test) {
						...
						// available methods are: onbeforeunload, log, error, result, complete, info
						if (!result.success) {
							console.log("SCREENSHOT - test", arguments);
							karma.info({
					...
					var fnOriginalPush = QUnit.assert.push;
					QUnit.assert.push = function (result, actual, expected, message) {
						if (!result) {
							console.log("SCREENSHOT - assert", arguments);
							karma.info({
								action: "take_screenshot",
								data: {
									assert: {
						...
						return fnOriginalPush.apply(this, arguments);
					};

					var fnOriginalPushFailure = QUnit.pushFailure;
					QUnit.pushFailure = function (message, source, actual) {
						console.log("SCREENSHOT - exception", arguments);
						karma.info({
							action: "take_screenshot",
							data: {
								failure: {
						...
						return fnOriginalPushFailure.apply(this, arguments);
					}
				});
			}
